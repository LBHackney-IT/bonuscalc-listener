...
data "aws_ssm_parameter" "repairs_sns_topic_arn" {
  name = "/sns-topic/staging/repairs/arn"
}
...
resource "aws_ssm_parameter" "repairs_sqs_queue_arn" {
   name  = "/sqs-queue/staging/repairs/arn"
   type  = "String"
...
resource "aws_cloudwatch_metric_alarm" "dlq_alarm" {
  alarm_name          = "DLQ_Alarm_${aws_sqs_queue.repairs_dead_letter_queue.name}_${var.environment_name}"
  ...
  dimensions = {
    QueueName = aws_sqs_queue.repairs_dead_letter_queue.name
  }
